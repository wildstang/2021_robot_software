...
public class PathReader {
    ...
    public static Trajectory readTrajectory(File p_path, boolean isForwards) {
        ...
        if (records.get(0).get("Delta Time").equals("dt")){
            records.remove(0);
        }
        ...
        for (CSVRecord record : records) {
            ...
            dataPoints[i][1] = modifier * Double.parseDouble(record.get("Position")) * DriveConstants.TICKS_PER_INCH_MOD*12;//*8.923;
            dataPoints[i][2] = modifier * Double.parseDouble(record.get("Velocity")) * DriveConstants.TICKS_PER_INCH_MOD/10*12;//*8.923;
