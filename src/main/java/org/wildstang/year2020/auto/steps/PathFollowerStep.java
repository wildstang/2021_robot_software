...
public class PathFollowerStep extends AutoStep {
    ...
    public PathFollowerStep(String p_path, boolean isLeft, boolean isForwards) {
        ...
        SmartDashboard.putString("Testing path loading",Filesystem.getDeployDirectory().toString() + "/paths/"+p_path);

        m_filePath = Filesystem.getDeployDirectory().toString() + "/paths/" + p_path;
    ...
    public void initialize() {
        ...
        File leftFile = new File(m_filePath + ".left.csv");
        File rightFile = new File(m_filePath + ".right.csv");
